if isempty(gcbo)
	...
	fin.tokamak=uicontrol('units','normalized','style','popupmenu','tag','tokamak','position',[0.01 0.9 0.09 0.05],...
	'string','DIII-D|JET|TCV','callback','damw');
	...
	fin.dia1=uicontrol('units','normalized','position',[0.24 0.9 0.15 0.05],'string','PCI','callback','riflX riflx');
	fin.dia11=uicontrol('units','normalized','tag','PCI','position',[0.24 0.85 0.15 0.05],'string','Analysis options','callback','A_opzioni');
	...
	switch tokamak
		...
		case 3
		   ...
		   set(fin.dia2,'string','Reflectometer');
		   fin.ref=uicontrol('units','normalized','style','edit','tag','freq','position',[0.48 0.79 0.1 0.05],'string','Reflect freq.');
	end	
end
